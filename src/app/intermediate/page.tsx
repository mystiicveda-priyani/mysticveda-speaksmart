// Fetch intermediate words from Supabase table named "words".
// Only fetch rows where level = "intermediate".
// Show 5 random words.
...
export default function IntermediatePage() {
  ...
  const loadWords = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await fetchRandomWords("intermediate", 5);
      setWords(data);
    } catch (err) {
      ...
      <div className="relative z-10 mx-auto max-w-6xl px-6 py-10 sm:py-14">
        ...
        {loading && !error ? (
          ...
          words.length === 0 && (
            <p className="mt-12 text-center text-slate-500">
              No intermediate words found in the database yet.
            </p>
          )
        )}
        ...
        <div className="mt-12 flex justify-center">
          <button
            ...
            onClick={loadWords}
            disabled={loading}
            className="rounded-full bg-gradient-to-r from-amber-500 to-orange-500 px-8 py-3 text-sm font-semibold text-white shadow-lg shadow-amber-200 transition-transform hover:scale-105 disabled:cursor-not-allowed disabled:opacity-60 sm:text-base"
          >
